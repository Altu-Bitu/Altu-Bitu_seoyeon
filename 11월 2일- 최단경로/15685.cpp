...
using namespace std;
typedef pair<int, int> ci; // 좌표 저장을 위한 자료형 ci 선언
...
int cntSquare(vector<vector<bool>>& board) {
    ...
    int ans = 0;
    for (int i = 0; i < SIZE - 1; i++) { // i = 0 ~ SIZE-2
        for (int j = 0; j < SIZE - 1; j++) { // j = 0 ~ SIZE-2 

            //차례대로 왼쪽 위, 왼쪽 아래, 오른쪽 위, 오른쪽 아래
            if (board[i][j] && board[i][j + 1] && board[i + 1][j] && board[i + 1][j + 1]) // 네 꼭짓점이 드래곤 커브에 속해있다면
                ans++; // 개수 증가 
        }
    }
    return ans; // 리턴
...
ci makeDragonCurve(vector<vector<bool>>& board, vector<int>& direc, ci point) {
    ...
    for (int i = size - 1; i >= 0; i--) { // size-1회 loop.
        int new_direc = (direc[i] + 1) % 4; //현재 시작 점의 방향
        point = { point.first + dy[new_direc], point.second + dx[new_direc] }; //현재 끝 점
        board[point.first][point.second] = true; // 현재 끝점에 드래곤 커브 안착.
        direc.push_back(new_direc); // 다음 방향 알려주기.
    }
    return point;// 끝 점 리턴
}
...
int main() {
    int n, x, y, d, g; //(x,y) -> 드래곤 커브 시작점 , d->시작방향 ,g->세대
