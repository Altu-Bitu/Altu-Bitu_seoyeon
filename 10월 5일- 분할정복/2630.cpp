...
bool conquer(int size, int row, int col, int color) { // (row,col)에서 size만큼의 종이의 색이 모두 같은지 확인
...
vector<int> divide(int size, int row, int col) {
    vector<int> combine = { 0, 0 }; // 색상이 0인 종이의 수, 색상이 1인 종이의 수
...
int main() {
    ...
    //입력
    cin >> n;
    paper.assign(n, vector<int>(n, 0));//2차원 배열 초기화
