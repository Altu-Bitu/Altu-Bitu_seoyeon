...
// 하 상 좌 우 
int dr[4] = { -1, 1, 0, 0 }; //행 증가량
int dc[4] = { 0, 0, -1, 1 }; //열 증가량

vector<vector<ci>> findCard(vector<vector<int>>& board) { 
    vector<vector<ci>> card_pos(7, vector<ci>(0)); //최대 카드 수
    int cnt = 0; //실제 존재하는 카드의수 

    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {

            cnt = max(cnt, board[i][j]); //기존 cnt VS 카드 번호 중 더 큰 값
            if (board[i][j]) //카드라면 위치 저장
                card_pos[board[i][j]].emplace_back(i, j); // 카드위치쌍 (i,j) 저장.
        }
    }
    card_pos.resize(cnt + 1); //실제 존재하는 카드의 수만큼 크기 조절
    return card_pos; //존재하는 모든 카드의 위치를 리턴
}
...
pair<int, int> ctrl(int row, int col, int dir, vector<vector<int>>& tmp) { //컨트롤로 이동하는 좌표를 리턴하는 함수
    ...
    while (true) {
        ...
        if (row < 0 || row >= SIZE || col < 0 || col >= SIZE) // 해당 방향에 카드가 하나도 없다면 
            return make_pair(row - dr[dir], col - dc[dir]); //그 방향의 가장 마지막 칸으로 이동

        if (tmp[row][col] != 0) //카드가 존재한다면 
            return make_pair(row, col); //누른 키 방향에 있는 가장 가까운 카드 리턴
    }
}

int bfs(int r1, int c1, int r2, int c2, vector<vector<int>>& tmp) { //현재 커서에서 목표 카드로 이동하는 가장 적은 비용을 리턴하는 함수

    vector<vector<int>> visited(SIZE, vector<int>(SIZE, 0)); //방문체크 겸 dist 역할 
    queue<pair<int, int>> q;//큐
    visited[r1][c1] = 1; //반드시 엔터를 누르게 될 것. 엔터를 미리 눌렀다 가정하고 1로 표시
    q.push({ r1, c1 }); //현재 커서 위치 푸시

    while (!q.empty()) { //큐가 비어있지 않을동안 (커서위치가 소진되지 않을동안)


        //현재 정점
        int row = q.front().first;  //열
        int col = q.front().second; //행
        ...
        for (int i = 0; i < 4; i++) { //컨트롤 입력 이동 : 상,하,좌,우
            pair<int, int> np = ctrl(row, col, i, tmp);

            if (visited[np.first][np.second]) //ctrl로 이동한 점 = 0 (카드가 없다면)
                continue; //넘김

            visited[np.first][np.second] = dist + 1; // 거리dp 갱신.
            q.push(np);//큐에 푸시
        }

        for (int i = 0; i < 4; i++) { //방향키 입력 이동
            int nr = row + dr[i], nc = col + dc[i]; //다음 탐색할 좌표의 위치

            if (nr < 0 || nr >= 4 || nc < 0 || nc >= 4 || visited[nr][nc]) //범위를 벗어나거나 이미 방문했다면
                continue; //넘김 

            visited[nr][nc] = dist + 1; // (nr,nc) 까지의 거리 저장.
            q.push({ nr, nc }); // 다음 정점 푸시 
        }
    }
...
int matchCard(int bit, int num, int r, int c, vector<int>& seq, vector<vector<ci>>& cards, vector<vector<int>> tmp) { //조합에 대해 카드를 매칭하는 함수
    ...
    return ans; //현재 커서에서 목표 카드까지 이동하는 비용 리턴
}

int solution(vector<vector<int>> board, int r, int c) {
    int answer = 1e9; // 모든 카드를 제거하기 위한 키 조작 횟수의 최솟값 초기화. 
    ...
    */
    ...
    } while (next_permutation(seq.begin(), seq.end())); //가능한 모든 카드의 순서가 존재할 동안.
    return answer;
}
