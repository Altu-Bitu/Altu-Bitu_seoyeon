...
int bfs(int n, int m, int cnt, vector<vector<int>>& matrix, queue<ci> q) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) { //상, 하, 좌, 우 방향 탐색(인접한 노드 탐색)
            ...
            //범위체크 : 1<=nr<=n, 1<=nc<=nc + 토마토체크 
            if (nr > 0 && nr <= n && nc > 0 && nc <= m && matrix[nr][nc] == 0) { //노드가 존재(행렬 범위 안에 있음), 익지 않은 토마토라면
              
                //현재는 토마토가 익었으니까 1부터 시작하므로 마지막에 t-1
                matrix[nr][nc] = matrix[cr][cc] + 1; //시간 증가값을 토마토 행렬에 저장 -> 따로 방문체크하는 배열을 만들지 않아도 됨.
                q.push(ci(nr, nc));
            }
        }
    }

    if (!cnt) //모든 토마토가 익었다면 cnt=0이라면
        return t - 1;
    return -1;
}
