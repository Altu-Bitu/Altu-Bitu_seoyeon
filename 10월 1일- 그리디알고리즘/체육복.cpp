...
int solution(int n, vector<int> lost, vector<int> reserve) {
    vector<int> student(n + 1, 1); //학생마다 체육복 개수 기본 1개.
    ...
    for (int i = 1; i <= n; i++) { //1번 학생부터 그리디적으로 양 옆으로 체육복을 빌릴 수 있다면 빌리기
        ...
        //이때, 앞 사람부터 시작하므로 꼭 왼쪽 -> 오른쪽 순서로 체육복을 빌릴수있는지 체크해야한다. 
 
        if (student[i - 1] == 2) { //왼쪽 학생이 여벌 체육복을 가지고 있다면
            student[i - 1]--;
            student[i]++; //왼쪽 학생한테 체육복 빌리기
        }
        else if (student[i + 1] == 2) { //오른쪽 학생이 여벌체육복을 가지고 있다면 
            student[i + 1]--;
            student[i]++; //오른쪽 학생한테 체육복 빌리기
        }
