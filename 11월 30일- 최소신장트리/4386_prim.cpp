...
typedef pair<double, int> ci; //우선순위 큐에 쓰일 (가중치,정점) 
...
double prim(int v, int start, vector<vector<ci>>& graph) {
    ...
    priority_queue<ci, vector<ci>, greater<>> pq; //minheap 

    //초기화
    dist[start] = 0;
    pq.push({ 0, start }); // 시작정점푸시 
...
int main() {
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
            ...
            //(가중치, 연결된 정점) 형태로 저장.
            graph[i].emplace_back(dist, j);  // i->j 를 잇는 가중치 dist의 간선
            graph[j].emplace_back(dist, i); // j->i 를 잇는 가중치 dist의 간선
        }
    }
