...
void post(int id,int prame,int t) {
	...
	if (recommend[id]) { // 1. 이미 후보로 올라와있는경우
 ...
 //최대정원에 도달하면 가장적은추천수 가진후보 찾고 교체.
	...
	for (int i = 1; i <= 100; i++) {
		...
		else if ((recommend[min] == recommend[i]) && (rtime[min] > rtime[i])) // 추천수가 같으면 추천 받은 시각이 더 이를때만
			min = i; //교체			
